...
Runs DEBUG=5 PROFILE=1 benchmark, then parses the output to produce:
...
NOTE on timing:
  - DEBUG=5 kernel times are REAL for warm-up (pre-JIT, token 1).
    After JIT, kernels run inside ICBs and DEBUG=5 stops printing per-kernel.
  ...
  - Metal source is shown for top 4 non-weight-loading kernels (excludes E_ elementwise).
"""
...
def print_report(data, profile, model_name, kernel_sources=None):
  ...
  if kernel_sources: print(f"  (kernel source captured for {len(kernel_sources)} kernels)")
  print("=" * W)
  ...
  if compute_kerns:
    ...
    # Show source for top 4 non-weight-loading kernels (exclude E_ elementwise)
    sources_shown = 0
    for i, (name, times) in enumerate(by_total[:20]):
      ...
      # Show source for top 4 non-weight-loading kernels (skip E_ elementwise kernels)
      if kernel_sources and sources_shown < 4 and name in kernel_sources and not name.startswith('E_'):
        print(f"\n    Source for {name}:")
...
def main():
  if len(sys.argv) < 2:
    ...
    print(f"  e.g.: {sys.argv[0]} glm-4.7:flash 20")
    sys.exit(1)
  ...
  env = os.environ.copy()
  env['DEBUG'] = '5'  # Always use DEBUG=5 to capture kernel source
  env['PROFILE'] = '1'
  for e in extra_env:
    k, v = e.split('=', 1)
    env[k] = v
  ...
  prof = load_profile()
  kernel_sources = extract_kernel_sources(all_output)
  ...
  print(f"    # Find kernel Metal source in debug log (already captured above)")
