...
**tinygrad generates 6-30x more kernels than llama.cpp for the same model.** Dispatch overhead is 60-80% of wall time. Individual kernel efficiency is already decent (100+ GB/s for Q4_0). The gap is almost entirely dispatch overhead, not kernel quality.

| | llama.cpp | tinygrad | Ratio |
|--|-----------|----------|-------|
| youtu kernels/tok | ~50-100 | 586 | 6-12x |
| GLM kernels/tok | ~200 | 1700 | 8.5x |
| per-kernel overhead | ~0 (pipelined) | ~34us (barrier-serialized) | -- |

### Current Performance

| Model | Params | tok/s | ms/tok | Kernels | Theoretical | Gap |
|-------|--------|-------|--------|---------|-------------|-----|
| youtu-llm:2b-Q4_0 | 0.69 GB | **56** | 17.8 | 586 | 145 (6.9ms) | 2.6x |
| deepseek-v2-lite | ~2 GB | **26.5** | 37.7 | 840 | — | — |
| GLM-4.7-Flash | 1.24 GB | **18** | 55.6 | 1700 | 80 (12.4ms) | 4.4x |

### Action Plan (Ranked by impact x feasibility)

#### 1. Metal ICB Barrier Removal — highest impact, moderate risk

Every kernel in an ICB gets `.setBarrier()`, serializing all kernels even when independent. llama.cpp's Metal graph pipelines independent ops.

**Change**: In `metal.py`, track buffer read/write dependencies. Only insert barriers between kernels that share a buffer where at least one writes. Independent kernels (q_a vs kv_a projections, gate vs up, etc.) run without barriers.

**Expected**: Per-kernel overhead 34us→15us. youtu: 586×15us+7ms = 15.8ms = **63 tok/s**. GLM: proportionally larger gain. Benefits ALL models.

#### 2. Custom `mul_mat_id` CompiledRunner for MoE — high impact for GLM

Proven pattern: Q4K MSL runner gave 1.8x, Q6K MSL gave 48%. Same approach for MoE expert dispatch: one kernel, per-expert Z-threadgroups, full SIMD reduction. Replaces gather + fused-dequant-matmul (2-13 GB/s) with ~60-80 GB/s.

Like llama.cpp's `mul_mv_id`: each Z-threadgroup = one (expert, token) pair with full 32×nsg thread parallelism.

#### 3. Scheduler Fusion — high impact, high risk

Current 18 kernels/layer (youtu) could be 10-11. Saving 7/layer × 32 = 224 kernels = 7.6ms.

Fusion targets:
- RMSNorm reduce + elementwise → 1 kernel (saves 2 per norm × 5 norms = 10/layer)
- QK + softmax → 1-2 kernels (saves 2-3/layer)
- Norm + following matmul input

Risk: Removing barriers explodes to 3600+ kernels. Need targeted patterns, not wholesale removal.

#### 4. Ushort Codegen for Q4_0 — moderate impact, low risk

At MetalRenderer level, pattern-match `LOAD(uint8)→BITWISE_AND/RSHIFT` and emit `LOAD(ushort)→extract 4 nibbles`. Q4_0 kernels 103-147 GB/s → 200+ GB/s. Saves ~3.5ms compute on youtu. Stacks with other changes.

#### The Math

All improvements combined (youtu):
- Barrier removal: 34us→15us per kernel
- Scheduler fusion: 586→362 kernels
- Ushort: 7ms→3.5ms compute
- Total: 362×0.015 + 3.5 = **8.9ms = 112 tok/s**

### Key Learnings Summary

**What worked**: CompiledRunner (1.8x), Q6K MoE MSL (48%), pairwise topk (29→3 kernels), MoE fusion break (41%), Q4_0 packed-dot (2x), selective .contiguous() split (27%), GROUPTOP=32 (+4%), Q4_0 format (+11-22%), single K cache (-130 kernels).

**What didn't work**: MOE_FUSED custom kernels (6x slower), per-expert dispatch (-18%), removing .contiguous() (3600+ kernels), split KV cache (+66 kernels), ushort tensor bitcast (wrong results), MV heuristic relaxation (1 match), BEAM on fused dequant (no help).

**Critical insights**:
1. Isolated micro-benchmarks don't predict full-model behavior
2. Barriers are necessary evils (without them: 60s scheduling or 3600+ kernels)
3. Scheduler fuses THROUGH .cat() but NOT through .realize()
4. Dispatch overhead dominates, not kernel quality
5. Fused dequant breaks the optimizer (`MUL(dequant_chain, INDEX)` ≠ `MUL(INDEX, INDEX)`)
6. Metal ICB profiler divides time evenly (warm-up times are the real measurements)
7. Q4_0 > Q4_K for MoE models (simpler dequant fuses better)

### Navigation

- **[Action Plan](#action-plan-ranked-by-impact-x-feasibility)** — what to do next
- **[Youtu Q4_0 Sprint](#youtu-q4_0-speed-sprint-265--70-toks-feb-7-2026)** — dense model optimization journal
- **[GLM-4.7-Flash Plan](#glm-47-flash--50-toks-plan)** — MoE model plan and profile
- **[Youtu Speed History](#youtu-llm-2b-q4-speed-history)** — landed changes and benchmarks
- **[Youtu Q4_0 Experiments (Feb 8)](#youtu-q4_0-experiments-feb-8)** — model-level experiments that hit the wall
- **[llama.cpp MoE Reference](#how-llamacpp-does-moe)** — their approach (from llamacpp.md)
...
## Youtu Q4_0 Experiments, Feb 8

(Consolidated from experiments.md)
...
(see Ushort Codegen Design section above)
...
## How llama.cpp Does MoE (reference: llamacpp.md)

llama.cpp does NOT fuse gate+silu+up+down+sum+residual. Separate ops:
```
1. gate = mul_mat_id(gate_exps, input, expert_ids)  // per-expert GEMV
2. up   = mul_mat_id(up_exps,   input, expert_ids)
3. act  = swiglu(gate, up)
4. down = mul_mat_id(down_exps, act,   expert_ids)
5. out  = down[:,0,:] + down[:,1,:] + ... + down[:,k-1,:]  // explicit adds
```
