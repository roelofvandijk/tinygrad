...
### Correctness + benchmark in one run
```bash
# stdout = generated text (correctness check), stderr = per-token timing
.venv2/bin/python tinygrad/apps/llm.py --model "youtu-llm:2b-Q4" --prompt "Hello" --count 10 --benchmark > ./combo_stdout.log 2> ./combo_stderr.log
```

### Benchmark only (no tokenizer)
```bash
