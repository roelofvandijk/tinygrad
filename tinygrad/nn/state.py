...
def ggml_data_to_tensor(t: Tensor, n: int, ggml_type: int) -> Tensor:
  ...
  if (nelements_nbytes := GGML_BLOCK_SIZES.get(ggml_type)) is not None:
    ...
    if ggml_type == 13:  # Q5_K: 256 elements per 176-byte block (d:2, dmin:2, scales:12, qh:32, qs:128)
      ...
      sc = s[:,0:4].bitwise_and(63).cat(s[:,8:12].bitwise_and(0xF).bitwise_or(s[:,0:4].rshift(6).lshift(4)), dim=-1)
      mn = s[:,4:8].bitwise_and(63).cat(s[:,8:12].rshift(4).bitwise_or(s[:,4:8].rshift(6).lshift(4)), dim=-1)
      qh = q_to_uint8(blocks[:,16:48], 1).reshape(-1, 8, 32)
      q = Tensor.stack((qs:=blocks[:,48:176].reshape(-1,4,32)).bitwise_and(0xF), qs.rshift(4), dim=2).reshape(-1,8,32)
      return (d * sc.unsqueeze(-1) * (q + qh * 16).cast(dtypes.float32) - dmin * mn.unsqueeze(-1)).flatten(-2)
    if ggml_type == 14:
...
@accept_filename
def gguf_load(tensor: Tensor, quantized: bool = False) -> tuple[dict, dict[str, Tensor], dict[str, tuple[Tensor, tuple, int]]]:
  """
  Loads a .gguf file, returning the `kv_data`, `state_dict`, and `quantized_tensors`.
  If quantized=True, quantized_tensors contains raw quantized blocks (tensor name -> (blocks, shape, ggml_type)), otherwise empty dict.
