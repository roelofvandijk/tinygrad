...
def fix_group_for_reduce(x:UOp):
  # Prefer direct grouped ranges on REDUCE.src[1:] when available.
  # This preserves exact axis structure and avoids accidental over-capture from symbolic expressions.
  # Fallback to symbolic range extraction only when grouped ranges were folded into expressions.
  reduce_gfr_direct, reduce_r_direct = partition(x.src[1:], lambda u: u.op is Ops.RANGE and u.arg[1] == AxisType.GROUP_REDUCE)
  if len(reduce_gfr_direct):
    reduce_gfr, reduce_r = list(reduce_gfr_direct), list(reduce_r_direct)
  else:
