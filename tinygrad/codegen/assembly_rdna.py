...
class RDNACodegen(AssemblyCodegen):
  ...
  def specialize(self, asm) -> Tuple[str, str]:
    ...
    for uop, out, vin, arg in asm:
      if uop is UOps.DEFINE_REGISTER:
        ...
        if arg in [BinaryOps.CMPLT, BinaryOps.CMPEQ]:
          ...
          if out.dtype == dtypes._float4:
            ...
            ins.append(f"{'s_' if out.scalar else 'v_'}{alu_arg}_{dtype_to_rdnatype[out.dtype] if arg is not UnaryOps.NOOP else 'b32'}{'_i24' if arg is BinaryOps.MUL and out.dtype != dtypes.float32 and not out.scalar else ''} {reg_out(out)}, {', '.join(reg_in(x) if x.__class__ is Register else str(x) for x in vin)}")
      ...
      elif uop is UOps.LABEL:
        ins.append(f"{arg}:")
      elif uop is UOps.COND_BRANCH:
