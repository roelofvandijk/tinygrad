...
from tinygrad.codegen.late.devectorizer import load_store_folding, load_store_indexing, devectorize, pm_reduce, \
  ReduceContext, correct_load_store, pm_render, pm_add_loads, pm_expand_index, pm_fold_expanded_index
...
def full_rewrite_to_sink(sink:UOp, ren:Renderer|None=None, optimize:bool=True) -> UOp:
  ...
  if DEVECTORIZE >= 2:
    pm_devectorize = sym+pm_expand_index+load_store_indexing+pm_fold_expanded_index+load_store_folding+load_store_indexing
  elif DEVECTORIZE:
    pm_devectorize = sym+devectorize+pm_expand_index+load_store_indexing+pm_fold_expanded_index+load_store_folding+correct_load_store+load_store_indexing
  else:
    pm_devectorize = sym+pm_expand_index+load_store_indexing+pm_fold_expanded_index+load_store_folding+correct_load_store+load_store_indexing
