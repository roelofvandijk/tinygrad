...
def lower_msl_sink_ast(ctx:list[Buffer], ast:UOp):
  ...
  if len(ast.src[0].arg) > 0 and ast.src[0].arg[0] == "q4_0_linear_msl":
    from tinygrad.apps.q4_linear_msl import lower_q4_linear_msl_ast
    return lower_q4_linear_msl_ast(ast, ctx[0].device)
