...
class QuantizedLinear:
  __slots__ = ('blocks', 'out_features', 'in_features', 'ggml_type', '_el_per_block', '_bytes_per_block', '_dequant_fn', 'name',
               '_q4k_blocks', '_dequant_cache')
  def __init__(self, blocks:Tensor, shape:tuple[int, int], ggml_type:int, name:str|None=None):
    self.blocks = blocks
...
class QuantizedExpertWeights:
  ...
  def _reshape_blocks(self) -> Tensor:
    if self.expert_first_in_memory:
      ...
      transposed = reshaped.permute(2, 0, 1, 3)
      return transposed.reshape(self.num_experts, self._blocks_per_expert, self._bytes_per_block)
  ...
  def __call__(self, sel:Tensor, x:Tensor) -> Tensor:
    ...
    blocks = self._reshape_blocks()
    selected_blocks = blocks[sel_flat]
    blocks_flat = selected_blocks.reshape(n_sel * self._blocks_per_expert, self._bytes_per_block)

    w_flat = self._dequant_fn(blocks_flat)
    # Q6K and Q5K benefit from splitting dequant/matmul fusion (~20% faster each in JIT steady-state)
    # Q4K prefers fusion (higher compression ratio makes materialization costlier)
    ...
    if padded_row_len > self.in_features:
      w = w[:, :, :self.in_features]
    x_float = x_flat.float().reshape(n_sel, 1, self.in_features)
    w_float = w.float()

    return (x_float @ w_float.transpose(-1, -2)).reshape(B, T, K, self.out_features)
