...
def fold_divmod_general(d: UOp, correct_divmod_folding: bool) -> UOp|None:
  ...
  if y.op is Ops.CONST and (c := y.arg) > 0:
    ...
    if d_op is Ops.MOD and x_min >= 0:
    ...
    factors = tuple(u.const_factor() for u in uops_no_const) if uops_no_const else ()
    if len(factors)==1 and (v:=uops_no_const[0].divides(factors[0])).vmax-v.vmin == 1:
    ...
    if not (x_min<0 and correct_divmod_folding):
      rems = [min((r:=f%c), r-c, key=abs) for f in factors]
      rem_min = rem_max = const % c
      for r,u,f in zip(rems, uops_no_const, factors):
        ...
        rem_min, rem_max = rem_min + r*lo, rem_max + r*hi
        if rem_max - rem_min >= c: break
      else:
        if (rem_min_floor := rem_min//c) == rem_max//c:
          if d_op is Ops.MOD:
            weighted_terms = [r * u.divides(f) for u, f, r in zip(uops_no_const, factors, rems) if r != 0]
            rem_expr = UOp.sum(*weighted_terms, x.const_like(const % c)) if weighted_terms else x.const_like(const % c)
          ...
          const_quot = (const - const%c + rem_min_floor*c)//c
    ...
    if x_min >= 0 and factors:
      from functools import reduce
      if (gcd_val := reduce(math.gcd, factors, c)) > 1:
        gcd = x.const_like(gcd_val)
        if (new_x := unwrap(x_peeled.divide_exact(gcd)).simplify() + (const%c)//gcd.arg).vmin >= 0:
    ...
    if d_op is Ops.IDIV and x_min >= 0:
  ...
  return new_x%y if d_op is Ops.MOD else new_x//y+(quo[0].sum(*quo[1:]) if quo else y.const_like(0))
