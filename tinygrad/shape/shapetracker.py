...
from typing import Tuple, List, Optional, Dict, Set, Iterable, cast, Union, Any
...
class ShapeTracker(tuple):
  def __add__(self, b:Union[ShapeTracker, Tuple[Any, ...]]) -> ShapeTracker:
    assert all(isinstance(x,View) for x in b)
    ret = self
    for v in b: ret = ShapeTracker((*ret, v)).simplify() # one view at a time = better simplification
    return ret

  def invert(self, out_shape:Tuple[sint, ...]) -> Optional[ShapeTracker]:
    inverted_views:List[View] = []
    for v,s in zip(self[::-1], [x.shape for x in self[::-1][1:]]+[out_shape]):
  ...
  @property
  def contiguous(self) -> bool: return len(self) == 1 and self[0].contiguous

  @property
  def consecutive(self) -> bool: return len(self) == 1 and (v:=self[0]).mask is None and v.strides == strides_for_shape(v.shape)

  @property
  def shape(self) -> Tuple[sint, ...]: return self[-1].shape

  @property
  def size(self) -> int: return self[-1].size()
  ...
  def vars(self) -> Set[Variable]: return set.union(*[v.vars() for v in self], set())

  @property
  def var_vals(self) -> Dict[Variable, int]: return merge_dicts([dict([v.unbind()]) for v in self.vars()])

  def unbind(self) -> Tuple[ShapeTracker, Dict[Variable, int]]:
    unbound_views, var_vals = zip(*[v.unbind() for v in self])
    return ShapeTracker(tuple(unbound_views)), merge_dicts(var_vals)

  # NOTE: if a stride is not always valid, it will be None
  def real_strides(self, ignore_valid=False) -> Tuple[Optional[sint], ...]:
    if len(self) == 1 and self[-1].mask is None: return self[-1].strides
    idxs: List[Node] = [Variable(f"idx{i}", 0, s-1) for i,s in enumerate(self.shape)]
    idx, valid = self.expr_idxs(idxs)
    ret: List[Optional[sint]] = [None] * len(self[-1].shape)
  ...
  def expr_idxs(self, idxs:Optional[Iterable[Node]]=None) -> Tuple[Node, Node]:
    ...
    idx, valid = _expr_view(self[-1], idxs)
    for view in reversed(self[0:-1]):
  ...
  def simplify(self) -> ShapeTracker:
    if len(self) >= 2 and (new_view := self[-2] + self[-1]) is not None:
      return ShapeTracker(self[:-2] + (new_view,)).simplify()
    return self

  # *** under this line are the movement ops ***

  def pad(self, arg: Tuple[Tuple[sint, sint], ...]) -> ShapeTracker: return ShapeTracker(self[0:-1] + (self[-1].pad(arg), ))
  def shrink(self, arg: Tuple[Tuple[sint, sint], ...]) -> ShapeTracker: return ShapeTracker(self[0:-1] + (self[-1].shrink(arg), ))
  def expand(self, new_shape: Tuple[sint, ...]) -> ShapeTracker: return ShapeTracker(self[0:-1] + (self[-1].expand(new_shape), ))
  def permute(self, axis: Tuple[int, ...]) -> ShapeTracker: return ShapeTracker(self[0:-1] + (self[-1].permute(axis), ))
  def stride(self, mul: Tuple[int, ...]) -> ShapeTracker: return ShapeTracker(self[0:-1] + (self[-1].stride(mul), ))

  def reshape(self, new_shape: Tuple[sint, ...]) -> ShapeTracker:
    if getenv("MERGE_VIEW", 1) and (new_view := self[-1].reshape(new_shape)) is not None: return ShapeTracker(self[0:-1] + (new_view,))
    return ShapeTracker(self + (View.create(new_shape), ))
