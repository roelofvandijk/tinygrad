...
base_fxn_for_op: Dict[Op, Callable] = {
  BinaryOps.ADD: add, BinaryOps.SUB: sub, BinaryOps.MUL: mul, BinaryOps.DIV: truediv,
...
def axes_slice(strides): return [s for s in strides if s != 0], tuple(slice(None) if s != 0 else 0 for s in strides)
...
def einsum_mulacc(einsum, get_strides, expand):
  def mulacc(a, b, new_shape):
    ...
    out = [i for i,ns in enumerate(new_shape) if a.shape[i] == ns and (i in a_axes or i in b_axes)]
